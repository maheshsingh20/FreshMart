...
public class EmailService(IConfiguration config, ILogger<EmailService> logger)
{
    ...
    public Task SendOrderConfirmationAsync(string email, string firstName, string orderRef, decimal total,
        IEnumerable<(string name, int qty, decimal price)> items) =>
        SendAsync(email, firstName, $"Order Confirmed — #{orderRef}", $"""
            <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px">
              <h2 style="color:#16a34a">Order Confirmed ✅</h2>
              <p>Hi {firstName}, your order <strong>#{orderRef}</strong> has been placed successfully.</p>
    ...
    public Task SendOrderStatusAsync(string email, string firstName, string orderRef, string status) =>
        SendAsync(email, firstName, $"Order #{orderRef} — {status}", $"""
            <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px">
              <h2 style="color:#16a34a">Order Update 📦</h2>
              <p>Hi {firstName}, your order <strong>#{orderRef}</strong> status has been updated to:</p>
    ...
    public Task SendEmailVerificationOtpAsync(string email, string firstName, string otp) =>
        SendAsync(email, firstName, "Verify your FreshMart account", $"""
            <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px">
              <h2 style="color:#16a34a">Verify your email ✉️</h2>
              <p>Hi {firstName}, use the OTP below to verify your email. Expires in <strong>10 minutes</strong>.</p>
    ...
    public Task SendPasswordResetOtpAsync(string email, string firstName, string otp) =>
        SendAsync(email, firstName, "Reset your FreshMart password", $"""
            <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px">
              ...
              <p>Hi {firstName}, use the OTP below to reset your password. Expires in <strong>10 minutes</strong>.</p>
              <div style="margin:24px 0;text-align:center">
                <span style="display:inline-block;font-size:36px;font-weight:800;letter-spacing:12px;color:#111827;background:#f3f4f6;padding:16px 32px;border-radius:12px;border:2px dashed #d1d5db">{otp}</span>
              </div>
            </div>
